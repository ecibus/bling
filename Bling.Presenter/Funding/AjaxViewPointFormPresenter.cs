...
{
    public class AjaxViewPointFormPresenter : Presenter
    {
        private IAjaxView m_View;
        private IViewPointDao m_Dao;
        private string m_Path;

        public AjaxViewPointFormPresenter(IAjaxView view)
            : this(view, new ViewPointDao(DMDDataSession()))
        {
        }

        public AjaxViewPointFormPresenter(IAjaxView view, IViewPointDao dao)
        {
            m_View = view;
            m_Dao = dao;
        }
        ...
        {       
            string targetFile = String.Format("VP_{0}.csv", DateTime.Now.ToString("yyyyMMdd"));
            m_Path = path;

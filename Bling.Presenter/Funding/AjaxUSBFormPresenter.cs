...
{
    public class AjaxUSBFormPresenter : Presenter
    {
        private IAjaxView m_View;
        private IUSBDao m_Dao;
        private string m_Path;

        public AjaxUSBFormPresenter(IAjaxView view)
            : this(view, new USBDao(DMDDataSession()))
        {
        }

        public AjaxUSBFormPresenter(IAjaxView view, IUSBDao dao)
        {
            m_View = view;
            m_Dao = dao;
        }
        ...
        {       
            string targetFile = String.Format("USB_{0}.csv", DateTime.Now.ToString("yyyyMMdd"));
            m_Path = path;

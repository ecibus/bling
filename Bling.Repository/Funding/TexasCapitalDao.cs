...
{
    public interface ITexasCapitalDao
    {
        List<List<string>> GetData(string start, string end, string batchno);
    }

    public class TexasCapitalDao : AbstractDao<string, int>, ITexasCapitalDao
    {
        public TexasCapitalDao(ISession session)
            : base (session)
        {
        ...
        public List<List<string>> GetData(string start, string end, string batchno)
        {
            ...
            using (var cn = new SqlConnection(DMDDataConnectionString))
            {
                using (var cmd = new SqlCommand { Connection = cn })
                {
                    ...
                    cmd.CommandText = "xGEM_TexasCapitalData";
